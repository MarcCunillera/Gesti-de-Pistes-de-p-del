      ...
      >
        ...
        <div style={{ marginBottom: 20 }}>
          ...
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
            ...
            <span style={{ fontSize: 12, fontWeight: 600, background: "#f9fafb", color: "#6b7280", border: "1px solid " + (t?.border || "#e5e7eb") + ",", borderRadius: 20, padding: "2px 10px" }}>Organitza: {organitzador?.nombre}</span>
          </div>
        </div>
            ...
            {reserva.jugadores?.map(function(id, i) {
              ...
              return (
                ...
                <UserAvatar user={jugador} size={32} onClick={function() { if (onOpenUserProfile) onOpenUserProfile(jugador); }} />
                  ...
                  {i === 0 && <span style={{ fontSize: 11, background: "#fffbeb", color: "#d97706", border: "1px solid #fcd34d", borderRadius: 10, padding: "1px 7px", fontWeight: 600 }}>Organitzador</span>}
                  {id === session.id && i !== 0 && <span style={{ fontSize: 11, background: "#f0f9ff", color: "#0284c7", border: "1px solid #7dd3fc", borderRadius: 10, padding: "1px 7px", fontWeight: 600 }}>Tu</span>}
                </div>
              );
            })}
            {Array.from({ length: 4 - numJugadors }).map(function(_, i) {
              return (
                <div key={i} style={{ display: "flex", alignItems: "center", gap: 10, opacity: 0.45 }}>
                  ...
                  <span style={{ fontSize: 13, color: "#9ca3af" }}>Plaça lliure</span>
                </div>
              );
            })}
          </div>
        ...
        {!yaUnido && !esOrganitzador && teInvitacio && (
          <div style={{ background: "#f0fdf4", border: "1.5px solid #6ee7b7", borderRadius: 12, padding: "14px 16px", marginBottom: 12 }}>
            <div style={{ fontWeight: 700, color: "#065f46", fontSize: 14, marginBottom: 10 }}>🎉 T'han convidat a aquest partit!</div>
        ...
        {!yaUnido && !esOrganitzador && reserva.abierto && !teInvitacio && (
          teSolicitudPendent ? (
            <div style={{ background: "#fffbeb", border: "1.5px solid #fde68a", borderRadius: 10, padding: "12px 16px", marginBottom: 12, display: "flex", alignItems: "center", gap: 10, fontSize: 13, color: "#92400e", fontWeight: 600 }}>
              ...
              Sol·licitud enviada — esperant la confirmació de l'organitzador
            </div>
          ) : (
            <button
            ...
            >
              {lleno ? "Sense places disponibles" : "Sol·licitar unir-m'hi"}
            </button>
          )
        )}
