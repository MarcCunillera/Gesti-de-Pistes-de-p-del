...
export default function ReservaConfirmModal({ data, onClose, config, t }) {
  ...
  const titulo = abierto ? "Partit obert de pàdel" : "Reserva pista de pàdel - Torrelameu";
  const desc = "Pista de pàdel Torrelameu - La pleta";
    ...
    <div style={modalOverlay} onClick={onClose}>
      ...
      >
        ...
        <div style={{ padding: "24px 24px 20px" }}>
          {/* Tancar */}
          <button onClick={onClose} style={{ position: "absolute", top: 14, right: 14, background: C.surfaceAlt, border: `1px solid ${C.border}`, borderRadius: "50%", width: 30, height: 30, cursor: "pointer", fontSize: 18, color: C.secondary, display: "flex", alignItems: "center", justifyContent: "center" }}>×</button>
          ...
          <div style={{ marginBottom: 18 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap", marginBottom: 4 }}>
              ...
              <span style={{ fontSize: 11, fontWeight: 700, background: abierto ? (dark ? "rgba(59,130,246,.14)" : "#eff6ff") : (dark ? "rgba(34,197,94,.14)" : "#f0fdf4"), color: abierto ? (dark ? "#93c5fd" : "#1d4ed8") : (dark ? "#86efac" : "#15803d"), border: `1px solid ${abierto ? (dark ? "rgba(96,165,250,.32)" : "#93c5fd") : (dark ? "rgba(74,222,128,.32)" : "#86efac")}`, borderRadius: 20, padding: "2px 10px" }}>
            ...
            <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
              ...
              <span style={{ fontSize: 12, color: C.muted }}>Pista de pàdel Torrelameu - La pleta</span>
            </div>
          </div>
          ...
          <div style={{ background: C.surfaceAlt, border: `1px solid ${C.border}`, borderRadius: 10, padding: "14px 16px", marginBottom: 16 }}>
            <div style={{ display: "flex", gap: 14, alignItems: "center" }}>
              <div style={{ textAlign: "center", minWidth: 36, flexShrink: 0 }}>
                ...
                <div style={{ fontSize: 11, fontWeight: 700, color: C.secondary, textTransform: "uppercase" }}>{mes}</div>
              </div>
              ...
              <div>
                ...
                <div style={{ fontSize: 12, color: C.muted }}>{duracion} min · {hora} – {horaFi}</div>
              </div>
            </div>
          </div>

          {/* Botó Google Calendar */}
          <a
            href={googleCalendarUrl(fecha, hora, duracion, titulo, desc)}
            target="_blank"
            rel="noopener noreferrer"
            style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 8, width: "100%", padding: "10px", background: C.surface, border: `1.5px solid ${C.border}`, borderRadius: 9, fontWeight: 600, fontSize: 13, color: C.text, textDecoration: "none", marginBottom: 10, boxSizing: "border-box" }}
          ...
          <button
            onClick={onClose}
            style={{ width: "100%", padding: "10px", background: "transparent", color: C.muted, border: "none", borderRadius: 8, fontWeight: 600, fontSize: 13, cursor: "pointer" }}
          >
            Tancar
          </button>
        </div>
